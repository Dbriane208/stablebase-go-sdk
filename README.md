...
### Customer Payment (Mobile Wallet App)
...
func handleQRScan(qrData string) {
    // Initialize SDK with customer's private key
    client, _ := sdk.NewClient(sdk.Config{
        Network:    "base-sepolia",
        PrivateKey: customerPrivateKey,
    })

    // Parse the scanned merchant QR code
    merchant, _ := qrcode.ParseMerchant(qrData)
    fmt.Printf("Pay to: %s\n", merchant.Name)

    // Customer enters amount (e.g., from UI)
    amount := 5_000_000 // 5 USDC (6 decimals)

    // Step 1: Approve USDC spending
    _, _ = client.Customer.ApproveToken("USDC", amount)

    // Step 2: Create order
    orderId, _ := client.Customer.CreateOrder(
        merchant.MerchantId,
        "USDC",
        amount,
        "Coffee purchase",
    ...
    receipt, _ := client.Customer.PayOrder(orderId)
    fmt.Println("Payment successful!", receipt.TxHash)
}
```
...
func main() {
    ...
    // Listen for paid orders and settle them
    client.Events.Subscribe(sdk.EventOrderPaid, func(event OrderPaidEvent) {
        receipt, _ := client.Platform.SettleOrder(event.OrderID)
        fmt.Printf("Settled order %s, tx: %s\n", event.OrderID, receipt.TxHash)
    })
}
```
